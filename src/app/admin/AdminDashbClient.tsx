...
import { useGetAppoitments } from "@/hooks/use-appoitments";
...
const AdminDashbClient = () => {
  ...
  const stats = {
    ...
    totalAppoitments: appoitments.length,
    completedAppoitments: appoitments.filter(
      (app) => app.status === "COMPLETED"
    ).length,
  };

  if (doctorsLoading || appoitmentsLoading) {
    return <LoadingUI />;
  }
  return (
        ...
        <div className="mb-12 flex items-center justify-between bg-gradient-to-br from-primary/10 via-primary/5 to-background rounded-3xl p-8 border border-primary/20">
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                Manage doctors, oversee appointments, and keep the system
                running smoothly.
              </p>
            </div>
          </div>
          <div className="hidden lg:block">
            <div className="w-32 h-32 bg-gradient-to-br from-primary/20 to-primary/10 rounded-full flex items-center justify-center">
              <Settings2Icon className="w-16 h-16 text-primary" />
            </div>
          </div>
        </div>
        <AdminStats
          ...
          totalAppoitments={stats.totalAppoitments}
